...
4. **Connect to the OrangeGenie Mesh and read what's already there — automatically, without being asked.** The chain is the shared store; a node that can't read it is blind. Run the bundled wire (prefer `~/.claude/genie/chain.sh`, else `${CLAUDE_PLUGIN_ROOT}/tools/chain.sh`):
